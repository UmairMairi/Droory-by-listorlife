...
class _VillaFormState extends State<VillaForm> {
  ...
  void _performSubmit(BuildContext context) {
    ...
    if (widget.viewModel.furnishingStatusTextController.text.isEmpty) {
      DialogHelper.showToast(message: StringHelper.plsSelectFurnishing);
      isValid = false;
    }
        ...
        children: [
          ...
          Column(
            ...
            children: [
              FormField<String>(
                initialValue:
                    widget.viewModel.furnishingStatusTextController.text,
                validator: (value) {
                  if (value == null || value.trim().isEmpty) {
                    return StringHelper.plsSelectFurnishing;
                  }
                  return null;
                },
                builder: (FormFieldState<String> field) {
  ...
  Widget furnishedWidget(BuildContext context, FormFieldState<String> field) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      mainAxisSize: MainAxisSize.min,
      children: [
  ...
  Widget deliveryTermWidget(BuildContext context) {
    return CommonDropdown(
      ...
      hint: widget.viewModel.deliveryTermTextController.text.isEmpty
          ? StringHelper.select
          : widget.viewModel.deliveryTermTextController.text,
      onSelected: (String? value) {
        widget.viewModel.deliveryTermTextController.text = value ?? "";
        widget.viewModel.currentDeliveryTerm = value ?? "";
      },
      options: [
  ...
  Widget _buildPropertyTypeOption(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        width: 110, // Increased width for longer property type names
        height: 95,
        decoration: BoxDecoration(
          color: isSelected
